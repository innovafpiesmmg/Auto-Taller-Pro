...
- **Core Modules**:
    ...
    - **Quotes**: Creation with detailed line items (mano de obra, artículo, otros), real-time totals, buttons to create OR and Invoice from quote. **Bidirectional link with ORs**: `presupuestos.orId` (set automatically on OR creation) + `ordenesReparacion.presupuestoId` (set when creating OR from presupuesto). Presupuesto list shows "Crear OR" button or "Ver OR" link depending on link status. OR detail shows linked presupuesto banner with code and total.
